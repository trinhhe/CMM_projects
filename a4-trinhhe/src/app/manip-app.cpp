...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
        ...
        if (QUESTION_12) {
            ...
            // NOT DONE //
            VectorXd regularizer; 
            regularizer.setZero(u.size());
            
            // double a = 0.01;
            // double tresh_diff = 0.02;
            // auto left_o = sim->handleCenters[0];
            // auto right_o = sim->handleCenters[1];
            // auto l = left_o + seg2(u,0);
            // auto r = right_o + seg2(u,1);

            // auto diff = r-l;
            // if(abs(diff(0)) <= tresh_diff && abs(diff(1)) > tresh_diff) {

            // }
            // else if(abs(diff(0)) ) 
            // regularizer(5) = -u(5)/100;
            
            // // //     regularizer(4) = 100;
            // auto reg = get_x(regularizer);
            // O += reg.sum();
            // }
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				// TODO: MatrixXd dxdu = ...
                SparseMatrixd H = get_H(u, x);
				MatrixXd dxdu = solve_AX_EQUALS_B(H,dFdu);
				// TODO: DODu += ...
	...
	SparseMatrixd get_dFdu(const VectorXd &u, const VectorXd &x) const {
		...
		SparseMatrixd ret; ret.resize(x.size(), u.size()); {
			for (const auto &pin : sim->pins) { 
				...
				// checkMatrixXd(matFD(u, F_wrapper), dFdu, false, true);
