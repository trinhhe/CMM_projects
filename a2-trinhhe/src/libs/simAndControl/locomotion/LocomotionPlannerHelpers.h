    ...
    Trajectory3D generateLimbTrajectory(RobotLimb* limb,
        ...
        while (t < tEnd) {
            ...
            if (cpi.isStance()) {
                ...
                //TODO: here get offset of ground into finalEEPos
...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            V3D forward_direction_world = heading * robot->forward;
            V3D sideways_direction_world = heading * RBGlobals::worldUp.cross(robot->forward);

            pos = pos + dt*(vForward * forward_direction_world + vSideways * sideways_direction_world);/* + TODO: fix this */
            headingAngle = headingAngle + dt*turningSpeed;/* + TODO: fix this */
