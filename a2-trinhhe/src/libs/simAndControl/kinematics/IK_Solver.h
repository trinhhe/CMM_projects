    ...
    void solve(int nSteps = 10) {
        ...
        for (uint i = 0; i < nSteps; i++) {
            ...
            for(int j = 0; j < endEffectorTargets.size(); j++){
                J.setZero();
                P3D p = endEffectorTargets[j].p;
                RB *rb = endEffectorTargets[j].rb;
                P3D target = endEffectorTargets[j].target;
                gcrr.estimate_linear_jacobian(p, rb, J);
                // gcrr.compute_dpdq(p, rb, J);
                Matrix J2 = J.block(0,6,3,q.size() - 6);
                deltaq += (J2.transpose() * J2).ldlt().solve(J2.transpose() * (V3D(target - gcrr.getWorldCoordinates(p, rb))));
            }     
